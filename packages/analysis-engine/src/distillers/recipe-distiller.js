 ...
 */
function generateProblemSignature(problemStatement) {
 ...
 */
function extractProblemStatement(episode) {
    // Look for the first user message that defines the goal
    const goalMessage = episode.messages.find((m) => m.role === 'user' && m.content.length > 20);
    if (goalMessage) {
 ...
 */
function extractApproachFromMessages(messages, analysis) {
    const steps = [];
    let stepNum = 1;
 ...
 */
function extractCoreAction(content) {
 ...
 */
function extractGuidanceLists(episode, analysis) {
    const doList = [];
    const dontList = [];
    ...
    if (analysis?.hallucinationIndicators) {
        dontList.push(...analysis.hallucinationIndicators.map((h) => `Don't assume: ${h}`));
    }
 ...
 */
function extractFailurePatterns(episode, analysis) {
    const patterns = [];
    ...
    for (const message of episode.messages) {
        ...
        if (message.role === 'user') {
            ...
            if (/\b(doesn't work|wrong|not what|try again|that's not)\b/.test(content)) {
                patterns.push({
 ...
 */
function inferTaskType(episode) {
 ...
 */
async function distillWithLLM(episode, analysis) {
    const content = episode.messages
        .map((m, i) => `[${i}] ${m.role}: ${m.content.slice(0, 500)}`)
        .join('\n\n');
    const result = await analyze(RECIPE_DISTILLATION_PROMPT, content);
    return result.data;
}
/**
 * Distill a success recipe from an episode
 */
export async function distillRecipe(episode, analysis, config = {}) {
    ...
    try {
        ...
        // Calculate metrics
        const originalTokens = episode.messages.reduce((sum, m) => sum + estimateTokens(m.content), 0);
        recipe = {
            recipeId: `rec_${nanoid(12)}`,
 ...
 */
export function distillRecipeSync(episode, analysis, config = {}) {
    const fullConfig = { ...DEFAULT_CONFIG, ...config };
    if (episode.outcome !== 'success' && episode.outcome !== 'partial') {
        return null;
    }
 ...
 */
export function calculateCompressionRatio(episode, recipe) {
    const originalTokens = episode.messages.reduce((sum, m) => sum + estimateTokens(m.content), 0);
    const recipeTokens = estimateTokens(recipe.problemStatement) +
...
export function formatRecipeForDisplay(recipe) {
    ...
    for (const step of recipe.approach) {
        ...
        if (step.rationale)
            output += `   _Rationale: ${step.rationale}_\n`;
    }
    output += '\n';
...
//# sourceMappingURL=recipe-distiller.js.map