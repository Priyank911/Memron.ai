 ...
 */
function extractKeySentences(text, maxCount = 5) {
 ...
 */
function ensureKeywordsPreserved(original, compressed, keywords) {
    const missing = [];
 ...
 */
function compressHeuristic(content, config) {
 ...
 */
async function compressWithLLM(content, config) {
    const originalTokens = estimateTokens(content);
    const prompt = COMPRESSION_PROMPT.replace('{content}', content);
    const result = await analyze(prompt, `Target compression: ${config.targetRatio * 100}% of original.` +
 ...
 */
export async function compressContent(content, config = {}) {
 ...
 */
export function compressContentSync(content, config = {}) {
    const fullConfig = { ...DEFAULT_CONFIG, ...config };
    return compressHeuristic(content, fullConfig);
}
/**
 ...
 */
export async function compressBatch(contents, config = {}) {
    return Promise.all(contents.map((content) => compressContent(content, config)));
}
/**
 * Get compression summary
 */
export function getCompressionSummary(result) {
...
//# sourceMappingURL=compression-optimizer.js.map