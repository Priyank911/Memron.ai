 ...
 */
function areSameTopic(memory1, memory2) {
    // Same memory type increases likelihood
    if (memory1.memoryType !== memory2.memoryType) {
        return false;
    }
    // Check content similarity
    const similarity = calculateSimilarity(memory1.compressedContent, memory2.compressedContent);
    return similarity > 0.4;
}
/**
 * Detect contradiction patterns
 */
function detectContradictionPatterns(content1, content2) {
 ...
 */
function detectUpdateRelation(memory1, memory2) {
 ...
 */
function detectExtensionRelation(memory1, memory2) {
    const c1 = memory1.compressedContent.toLowerCase();
    const c2 = memory2.compressedContent.toLowerCase();
    ...
    if (shorterLength > 0 && overlap.length / shorterLength > 0.7) {
        const longerMemory = memory1.compressedContent.length > memory2.compressedContent.length
            ? memory1
            : memory2;
        return {
            isExtension: true,
 ...
 */
function detectRelationHeuristic(memory1, memory2) {
    ...
    const contradiction = detectContradictionPatterns(memory1.compressedContent, memory2.compressedContent);
 ...
 */
async function detectRelationLLM(memory1, memory2) {
 ...
 */
export async function detectConflict(memory1, memory2, config = {}) {
 ...
 */
export function detectConflictSync(memory1, memory2, config = {}) {
 ...
 */
export function suggestResolution(update, memory1, memory2) {
    const time1 = new Date(memory1.timestamp).getTime();
    const time2 = new Date(memory2.timestamp).getTime();
    let resolution;
    let confidence = 0.6;
    ...
    return {
        ...
        conflictType: update.relation === 'contradicts'
            ? 'contradiction'
            : update.relation === 'updates'
                ? 'update'
                : 'overlap',
        resolution,
        resolvedMemoryId: resolution === 'keep_newer'
            ? memory2.memoryId
            : resolution === 'keep_older'
                ? memory1.memoryId
                : undefined,
 ...
 */
export async function findAllConflicts(memories, config = {}) {
    const updates = [];
    // Compare each pair
    for (let i = 0; i < memories.length; i++) {
        for (let j = i + 1; j < memories.length; j++) {
            const update = await detectConflict(memories[i], memories[j], config);
            if (update) {
                updates.push(update);
            }
        }
 ...
 */
export function findAllConflictsSync(memories, config = {}) {
    const updates = [];
    for (let i = 0; i < memories.length; i++) {
        for (let j = i + 1; j < memories.length; j++) {
            const update = detectConflictSync(memories[i], memories[j], config);
            if (update) {
                updates.push(update);
            }
        }
