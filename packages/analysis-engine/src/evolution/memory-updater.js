 ...
 */
export function applyUpdate(existingMemory, newMemory, update) {
 ...
 */
export function mergeMemories(memory1, memory2) {
    const timestamp = new Date().toISOString();
    // Determine which is primary (higher confidence or more recent)
    const primary = memory1.confidence > memory2.confidence
        ? memory1
    ...
    const mergedCompressed = mergeContent(primary.compressedContent, secondary.compressedContent);
    // Merge graph links
 ...
 */
export function resolveConflict(resolution, memory1, memory2, config = {}) {
    const fullConfig = { ...DEFAULT_CONFIG, ...config };
    switch (resolution.resolution) {
        case 'keep_newer':
            const newer = new Date(memory1.timestamp) > new Date(memory2.timestamp) ? memory1 : memory2;
        ...
        case 'keep_older':
            const older2 = new Date(memory1.timestamp) < new Date(memory2.timestamp) ? memory1 : memory2;
 ...
 */
export function processUpdate(update, existingMemory, newMemory, config = {}) {
    const fullConfig = { ...DEFAULT_CONFIG, ...config };
 ...
 */
export function processBatchUpdates(updates, memoryMap, config = {}) {
    const results = [];
 ...
 */
export function expireMemories(memories, maxAgeMs = 30 * 24 * 60 * 60 * 1000 // 30 days default
) {
    const now = Date.now();
    const results = [];
 ...
 */
export function getUpdateSummary(results) {
    const counts = {
        created: 0,
        updated: 0,
        merged: 0,
