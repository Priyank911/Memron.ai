 ...
 */
export async function runPipeline(input, config = {}) {
    ...
    if (options.extractEpisodes !== false) {
        ...
        episodes.push({
            ...
            outcome: detectOutcome({ messages: input.messages }).outcome,
    ...
    if (options.extractMemories !== false) {
        if (fullConfig.parallel && fullConfig.useLLM) {
            const memoryPromises = episodes.map((ep) => extractMemories(ep, input.userId, {
    ...
    if (options.analyzeTrajectory !== false) {
        if (fullConfig.parallel && fullConfig.useLLM) {
            const analysisPromises = episodes.map((ep) => analyzeTrajectory(ep, { useLLM: fullConfig.useLLM }));
    ...
    if (options.distillRecipes !== false) {
        const successfulEpisodes = episodes.filter((ep) => ep.outcome === 'success' || ep.outcome === 'partial');
    ...
    if (options.extractEntities !== false) {
        if (fullConfig.parallel && fullConfig.useLLM) {
            const entityPromises = episodes.map((ep) => extractEntities(ep, input.userId, { useLLM: fullConfig.useLLM }));
 ...
 */
export function runPipelineSync(input, config = {}) {
 ...
 */
export async function quickAnalyze(messages, userId) {
    const sessionId = `session_${nanoid(8)}`;
    // Quick sync analysis
    const episodes = splitIntoEpisodesSync(sessionId, messages);
    const outcome = detectOutcome(episodes[0] || { messages });
    ...
    return {
        ...
        compressionRatio: totalOriginalTokens > 0 ? 1 - totalCompressedTokens / totalOriginalTokens : 0,
    };
}
/**
