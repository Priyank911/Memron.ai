 ...
 */
function detectEpisodeTypeFromContent(message, previousMessages) {
 ...
 */
function findHeuristicBoundaries(messages, config) {
    const boundaries = [];
 ...
 */
async function findLLMBoundaries(messages) {
    const conversationText = messages
        .map((m, i) => `[${i}] ${m.role}: ${m.content.slice(0, 500)}${m.content.length > 500 ? '...' : ''}`)
        .join('\n\n');
    const result = await analyze(EPISODE_SPLITTING_PROMPT, conversationText);
 ...
 */
function determineEpisodeType(messages, boundaryReason) {
    ...
    if (reasonLower.includes('fail'))
        return 'failed_attempt';
    if (reasonLower.includes('correct'))
        return 'correction';
    if (reasonLower.includes('clarif'))
        return 'user_clarification';
    if (reasonLower.includes('tool'))
        return 'tool_result';
    if (reasonLower.includes('final') || reasonLower.includes('solution'))
        return 'final_solution';
    if (reasonLower.includes('explor'))
        return 'exploration';
    if (reasonLower.includes('context') || reasonLower.includes('setup'))
 ...
 */
function determineOutcome(messages) {
    const lastUserMessage = [...messages].reverse().find((m) => m.role === 'user');
    if (lastUserMessage) {
        const content = lastUserMessage.content.toLowerCase();
        // Success indicators
        if (/\b(perfect|works|great|thanks|exactly|solved|fixed|done|awesome)\b/.test(content)) {
            return 'success';
        }
        // Failure indicators
 ...
 */
function createEpisodes(sessionId, messages, boundaries) {
    const episodes = [];
 ...
 */
export async function splitIntoEpisodes(sessionId, messages, config = {}) {
 ...
 */
export function splitIntoEpisodesSync(sessionId, messages, config = {}) {
...
export async function summarizeEpisode(episode) {
    ...
    const result = await analyze(prompt, content);
    return result.data.summary;
}
//# sourceMappingURL=episode-splitter.js.map