 ...
 */
function detectTrajectoryPointType(message, messageIndex, isFirst) {
    const content = message.content.toLowerCase();
    if (isFirst && message.role === 'assistant') {
        return { type: 'initial_plan', confidence: 0.8 };
    }
    for (const [pointType, patterns] of Object.entries(TRAJECTORY_INDICATORS)) {
 ...
 */
function determineImpact(message, nextMessages) {
    const content = message.content.toLowerCase();
    const nextContent = nextMessages.map((m) => m.content.toLowerCase()).join(' ');
    // Positive indicators
    if (/\b(works|success|perfect|great|exactly|solved|fixed)\b/.test(nextContent) &&
        message.role === 'assistant') {
        return 'positive';
    }
    // Negative indicators
    if (/\b(error|failed|wrong|doesn't work|broken|issue)\b/.test(nextContent) &&
        message.role === 'assistant') {
        return 'negative';
    }
 ...
 */
function calculateStepScores(messages) {
 ...
 */
function extractTrajectoryPointsHeuristic(messages) {
    const points = [];
 ...
 */
function detectCriteria(messages) {
 ...
 */
function detectTokenWaste(messages) {
    const wastePoints = [];
    for (let i = 0; i < messages.length; i++) {
 ...
 */
async function analyzeWithLLM(episode) {
    const content = episode.messages
        .map((m, i) => `[${i}] ${m.role}: ${m.content.slice(0, 500)}`)
        .join('\n\n');
    const result = await analyze(TRAJECTORY_ANALYSIS_PROMPT, content);
    return result.data;
}
/**
 * Analyze an episode's trajectory
 */
export async function analyzeTrajectory(episode, config = {}) {
 ...
 */
export function analyzeTrajectorySync(episode, config = {}) {
 ...
 */
export function findWinningBranch(analysis) {
    return analysis.trajectoryPoints.filter((p) => p.type === 'winning_branch' || (p.type === 'refinement' && p.impact === 'positive'));
}
/**
 * Get trajectory summary
 */
export function getTrajectorySimpleSummary(analysis) {
    const outcomeStr = analysis.outcome.toUpperCase();
    const pivotCount = analysis.trajectoryPoints.filter((p) => p.type === 'branch_change' || p.type === 'retry').length;
...
//# sourceMappingURL=trajectory-analyzer.js.map