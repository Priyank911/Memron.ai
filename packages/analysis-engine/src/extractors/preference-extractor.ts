...
const DEFAULT_CONFIG: Required<PreferenceExtractorConfig> = {
  minConfidence: 0.6,
  useHeuristics: true,
  useLLM: true,
};
...
function extractWithHeuristics(
  ...
  for (const message of messages) {
    ...
    for (const [category, patterns] of Object.entries(PREFERENCE_PATTERNS) as [
      PreferenceCategory,
      RegExp[],
    ][]) {
 ...
 */
function detectImplicitPreferences(
  messages: ConversationMessage[]
): ExtractedPreference[] {
  const preferences: ExtractedPreference[] = [];
...
async function extractWithLLM(
  ...
  const result = await analyze<LLMPreferenceResult>(PREFERENCE_EXTRACTION_PROMPT, content);
...
function mergePreferences(
  ...sources: ExtractedPreference[][]
): ExtractedPreference[] {
  const merged = new Map<string, ExtractedPreference>();
...
export async function extractPreferences(
  episode: Episode,
  config: PreferenceExtractorConfig = {}
): Promise<ExtractedPreference[]> {
  const fullConfig = { ...DEFAULT_CONFIG, ...config };
  const sources: ExtractedPreference[][] = [];
 ...
 */
export function extractPreferencesSync(
  episode: Episode,
  config: PreferenceExtractorConfig = {}
): ExtractedPreference[] {
 ...
 */
export function preferencesToMemoryContent(preferences: ExtractedPreference[]): string {
  return preferences
