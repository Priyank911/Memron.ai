 ...
 */
function formatEpisodeForExtraction(episode) {
 ...
 */
function inferMemoryType(content, role) {
    const lowerContent = content.toLowerCase();
    // Check for explicit preference statements
    if (/\b(i prefer|i like|i want|i need|don't like|avoid)\b/.test(lowerContent) &&
        role === 'user') {
        return { type: 'preference', confidence: 0.85 };
    }
    // Check for constraints
    if (/\b(must|should|cannot|shouldn't|required|forbidden|limitation)\b/.test(lowerContent)) {
    ...
    // Check for success
    if (/\b(works|success|solved|fixed|perfect|complete|done)\b/.test(lowerContent) &&
        role === 'user') {
        return { type: 'observed_success', confidence: 0.8 };
    }
    // Check for resolution
    if (/\b(the solution is|here's how|the fix is|final version)\b/.test(lowerContent) &&
        role === 'assistant') {
 ...
 */
function extractFromMessage(message, messageIndex, episodeOutcome) {
    const memories = [];
 ...
 */
async function extractWithLLM(episode, config) {
    const content = formatEpisodeForExtraction(episode);
    const result = await analyze(MEMORY_EXTRACTION_PROMPT, content);
 ...
 */
export async function extractMemories(episode, userId, config = {}) {
    const fullConfig = { ...DEFAULT_CONFIG, ...config };
    const timestamp = new Date().toISOString();
    // Extract using both heuristics and LLM
    const heuristicMemories = [];
 ...
 */
export function extractMemoriesSync(episode, userId, config = {}) {
    const fullConfig = { ...DEFAULT_CONFIG, ...config };
    const timestamp = new Date().toISOString();
    const heuristicMemories = [];
    for (let i = 0; i < episode.messages.length; i++) {
 ...
 */
export function calculateCompressionStats(originalMessages, memories) {
    const originalTokens = originalMessages.reduce((sum, m) => sum + estimateTokens(m.content), 0);
    const compressedTokens = memories.reduce((sum, m) => sum + estimateTokens(m.compressedContent), 0);
    return {
        originalTokens,
        compressedTokens,
...
//# sourceMappingURL=atomic-extractor.js.map